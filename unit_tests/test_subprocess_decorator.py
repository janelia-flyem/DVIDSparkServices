import unittest
from DVIDSparkServices.subprocess_decorator import execute_in_subprocess, _test_helper

class TestSubprocessDecorator(unittest.TestCase):    
    
    def test_basic(self):
        collected_output = []
        result = execute_in_subprocess(stdout_callback=collected_output.append)(_test_helper)(1,2,0)
    ...
    def test_error(self):
        collected_output = []
        try:
            test_helper_in_subprocess = execute_in_subprocess(stdout_callback=collected_output.append)(_test_helper)
    ...
    def test_timeout(self):
        try:
            _result = execute_in_subprocess(lambda msg: None, timeout=1.0)(_test_helper)(1,2,3)
        except RuntimeError:
            pass
        else:
